...
class WeatherService {
  ...
  static Future<WeatherCondition> getCurrentWeather(Position? position) async {
    ...
    try {
      ...
      if (position != null) {
        ...
        bool isInTaiwan = position.latitude >= 21.0 && position.latitude <= 26.0 &&
  ...
  static Future<WeatherCondition> _fetchGlobalWeather(Position position) async {
    final url = Uri.parse('https://api.open-meteo.com/v1/forecast?latitude=${position.latitude}&longitude=${position.longitude}&current=weather_code');
