...
- **敘事型行銷與留白設計 (Storytelling UX)**：在官方網站或文案設計中，避免「一進門就強迫推銷或要求下載 (Hard Sell)」。必須透過文字鋪陳與視覺圖片建立情感連結，引導使用者自然向下探索（例如在手機版排版上，維持「故事文字 -> 情境截圖 -> 下載按鈕」的順序，讓使用者被故事打動後才看見按鈕）。
- **網頁極簡設計原則 (KISS Principle & Breathing Space)**：處理網頁端 UI 時，拒絕過度設計 (Over-engineering)。優先採用最穩定的靜態排版，避免依賴複雜的 JavaScript 滾動監聽或花俏的浮動特效。重視版面的「留白與呼吸空間」（例如維持左右寬裕的 padding），不盲目追求擁擠的滿版 (Full-bleed) 沉浸設計。同時需嚴格確保跨平台瀏覽器（尤其是 iOS Safari）的基礎互動相容性。
